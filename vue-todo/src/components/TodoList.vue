<template>
  <div class="list">
    <TransitionGroup name="list" tag="ul" v-if="this.$store.state.todoItems.length">
      <li v-for="(a, i) in this.$store.state.todoItems" :key="a.time" class="shadow" :class="{ completed: a.completed }">
...
export default {
  methods: {
    removeTodo (a) {
      this.$store.commit('removeTodo', { todoItem: a });
    },
    toggleComplete (a) {
      this.$store.commit('toggleComplete', { todoItem: a });
    }
  },
}
</script>
